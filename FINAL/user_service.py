...
class UserService:
    ...
    def _fetch_with_retry(self, user_id: int) -> Optional[dict]:
        ...
        logger.error(
            f"Usuario ID {user_id} no disponible después de {self.config.max_retries} intentos"
        )
        return None
    ...
    def generate_userlist(self, start_id: int, end_id: int) -> BytesIO:
        ...
        for user_id in range(start_id, end_id + 1):
            try:
                ...
                logger.error(
                    f"Error crítico procesando ID {user_id}: {str(e)}"
                )
            finally:
                total_processed += 1
                if total_processed % 100 == 0:
                    logger.info(
                        f"Progreso: {total_processed/(end_id-start_id+1)*100:.1f}%"
                    )
        ...
        logger.info(
            f"Generación completada. Usuarios válidos: {valid_users}/{total_processed}"
        )
        return byte_buffer
