            ...
            for table in table_2:
                ...
                if row:
                    ...
                    if len(cells) >= 5:
                        ...
                        anime_title_link = anime_title_cell.find(
                            "a", class_="animetitle"
                        )
                        anime_id = (
                            anime_title_link["href"].split("/")[2]
                            if anime_title_link
                            else ""
                        )
                        anime_title = (
                            anime_title_link.find("span").text.strip()
                            if anime_title_link
                            else ""
                        )

                        score_label = score_cell.find(
                            "span", class_="score-label"
                        )
                        score = (
                            score_label.text.strip() if score_label else "-"
                        )
...
def fetch_user_scores():
    with open(
        "userlist/userlist.csv", "r", newline="", encoding="utf-8"
    ) as file:
    ...
    min_delay_seconds = (
        90  # Minimum delay duration between requests in seconds
    )
    max_delay_seconds = (
        120  # Maximum delay duration between requests in seconds
    )
        ...
        for i in range(0, len(usernames), batch_size):
            ...
            for username, user_id in zip(usernames_batch, user_ids_batch):
                ...
                if data:
                    ...
                    print(
                        f"User details fetched successfully for username: {username}"
                    )
            ...
            if i + batch_size < len(usernames):
                ...
                print(
                    f"Waiting for {delay_seconds} seconds before the next batch..."
                )
                time.sleep(delay_seconds)
