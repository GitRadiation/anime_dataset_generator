...
def clean_string_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in str_cols:
        ...
        df[col] = (
            ...
            .replace(['\\N', 'nan', 'null', 'None'], pd.NA)  # Replace placeholders
        )
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        duration_fill = df['duration'].median() if df['duration'].notna().any() else 20
        episodes_fill = df['episodes'].median() if df['episodes'].notna().any() else 12
        ...
        if not np.isfinite(df[['duration', 'episodes']].values).all():
            ...
            logger.warning(f"Found non-finite values - Duration: {invalid_duration}, Episodes: {invalid_episodes}")
        ...
        logger.error(f"Preprocessing failed. Current data state:\n{df.head().to_string()}")
