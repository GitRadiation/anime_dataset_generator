...
class GeneticRuleMiner:
    ...
    def _create_condition(self, col: str) -> tuple[str, str, Union[float, str]]:
        ...
        if col in self._numeric_cols:
            ...
            unique_vals = [set(self.df[col].dropna())]  # No modificamos el DataFrame original
    ...
    def _vectorized_support(self, conditions: Sequence[tuple]) -> np.ndarray:
        mask = np.ones(len(self.df), dtype=bool)  # Inicializar la máscara booleana con 'True' para todas las filas
        ...
        assert len(mask) == len(self.df), f"Longitud de la máscara no coincide con el DataFrame: {len(mask)} != {len(self.df)}"
    ...
    def _vectorized_confidence(self, rule: dict) -> float:
        ...
        if np.sum(conditions_mask) == 0:  # Si no hay filas que cumplan las condiciones, la confianza es 0
            return 0
        ...
        if len(conditions_mask) != len(target_mask):
            raise ValueError(f"Las longitudes de las máscaras no coinciden: {len(conditions_mask)} != {len(target_mask)}")
